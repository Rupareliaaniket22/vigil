...
/// deliberate: stated this way they describe the behaviour and not the
/// mechanism, which is the only way to show they catch the defect. Pointed at
/// `f353192~1`, all four "keeps the script" cases fail and the three below
/// them still pass — so the suite is catching the defect rather than
/// describing whatever the code happens to do. That experiment now needs the
/// installer's `home` parameter carried back with it, since the commit
/// predates it; nothing else about the suite changes.
///
/// Not serialized, and no longer able to be. `FakeHome` used to move a
/// process-global environment variable, which two tests could not do at once;
/// it now hands each test a directory of its own.
@Suite("The hook script every agent shares")
struct SharedHookScriptTests {
  ...
  func unparseableSiblings() throws {
    #expect(
      try uninstallClaudeCode { home in
        // Three different ways `readSettings` refuses, one per agent.
        try write("{\"hooks\": ", to: home.settingsPath(for: .codex))
        try write("{\n  // vigil\n  \"hooks\": {}\n}", to: home.settingsPath(for: .gemini))
        try write("[1, 2, 3]", to: home.settingsPath(for: .cursor))
      } == .kept)
  }
  ...
  func unreadableSiblings() throws {
    #expect(
      try uninstallClaudeCode { home in
        for agent in siblings { try chmod(0o000, home.settingsPath(for: agent)) }
      } == .kept)
  }
    ...
    #expect(
      try uninstallClaudeCode { home in
        for agent in siblings {
          try chmod(0o000, (home.settingsPath(for: agent) as NSString).deletingLastPathComponent)
        }
      } == .kept)
  }
  ...
  func oneUnreadableSiblingAmongAbsentOnes() throws {
    #expect(
      try uninstallClaudeCode { home in
        for agent in siblings { try remove(home.settingsPath(for: agent)) }
        try write("{\"hooks\": ", to: home.settingsPath(for: .codex))
      } == .kept)
  }
  ...
  func installedSiblings() throws {
    #expect(try uninstallClaudeCode { _ in } == .kept)
  }
  ...
  func noSiblingUsesIt() throws {
    #expect(
      try uninstallClaudeCode { home in
        for agent in siblings { try write("{}", to: home.settingsPath(for: agent)) }
      } == .deleted)
  }
  ...
  private func uninstallClaudeCode(
    disturb: (FakeHome) throws -> Void
  ) throws -> Outcome {
    try FakeHome.run { home in
      try writeScript(at: home.scriptPath)
      for agent in AgentIntegration.all { try writeInstalled(agent, in: home) }
      ...
      return FileManager.default.fileExists(atPath: home.scriptPath) ? .kept : .deleted
    }
  }
