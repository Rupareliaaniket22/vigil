...
| 5 | Hook install into an agent's settings | `make smoke` runs a full install/uninstall cycle |
...
| 15 | `make integration` — 19 live checks, isolated | all passing, 24 September 2026 |
...
against a throwaway home with automatic hook management off, and checks
afterwards that the five real config files and the three preferences it could
otherwise have reached are byte-identical. That is what "isolated" means in the
row above, and it is the reason the number beside it can be trusted again.
...
- Launch a fresh Vigil against a machine with agents installed, confirm the
  panel's "Vigil set up …" notice names them, then press **Undo** and confirm
...
`make test && make lint && make smoke` all pass before committing, and
`make integration` too for anything touching the bridge, the model or the
policy — it is safe to run on your own machine again. A step that cannot be
verified is not finished — and if something cannot be verified in this
environment, say so in the commit rather than implying it works.
